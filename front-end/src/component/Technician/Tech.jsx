...
function TicketTech() {
     ...
     const resolvedTicket = (id) => {
          ...
          axios.post(`http://localhost:3001/api/resolvedTicket/${id}`).then(response => {
               toastr.success ('Ticket Resolved')
               console.log(response)
          })
     }
     ...
     return (
          ...
          <NavTech/>
           <div>
            <Layout title="Welcome To Technician Dashboard"></Layout>
          </div>
                              ...
                              {ticketList.map((Data, key) => (
                                   <tr key={key}>
                                        ...
                                        <td className="grid-action">
                                             <button type="button" className="btn-outline-info btn-sm ml-5" onClick={()=> {resolvedTicket( Data.id_ticket._id)}} >Accept</button>
                                             <button type="button" className="btn-outline-danger btn-sm ml-5" onClick={()=> {refuseTicket(Data.id_ticket._id)}}  >Refuse</button>      
                                        </td> 
